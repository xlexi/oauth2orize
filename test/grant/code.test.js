...
describe('grant.code', function() {
  ...
  describe('request parsing', function() {
    ...
    describe('request', function() {
      ...
      before(async function(done) {
        ...
        try {
          out = await server._parse('code', ctx);
        } catch(e) {
          err = e;
        }
    ...
    describe('request with list of scopes using scope separator option', function() {
      ...
      before(async function(done) {
        var server = new Server();
        server.grant(code({ scopeSeparator: ',' }, issue));
  ...
  describe('decision handling', function() {
    function issue(client, redirectURI, user) {
    ...
    describe('transaction', function() {
      ...
      it('should respond', function() {
        ...
        expect(ctx.response.get('Location')).to.equal('http://www.example.com/auth/callback?code=xyz');
      });
    });
    ...
    describe('transaction with request state', function() {
      ...
      it('should respond', function() {
        expect(ctx.status).to.equal(302);
        expect(ctx.response.get('Location')).to.equal('http://www.example.com/auth/callback?code=xyz&state=f1o1o1');
      });
    });
  ...
  describe('decision handling with user response', function() {
    function issue(client, redirectURI, user, ares) {
  ...
  describe('decision handling with user response and client request', function() {
    function issue(client, redirectURI, user, ares, areq) {
  ...
  describe('decision handling with response mode', function() {
    ...
    describe('transaction using default response mode', function() {
      ...
      before(async function(done) {
        var server = new Server();
        server.grant(code({ modes: { foo: fooResponseMode } }, issue));
      ...
      it('should respond', function() {
        expect(ctx.status).to.equal(302);
        expect(ctx.response.get('Location')).to.equal('http://www.example.com/auth/callback?code=xyz&state=s1t2u3');
      });
    });
