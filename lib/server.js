 ...
 * @api public
 */
Server.prototype.serializeClient = async function(fn, done) {
 ...
 * @api public
 */
Server.prototype.deserializeClient = async function(fn, done) {
  if (typeof fn === 'function') {
...
Server.prototype._parse = async function(type, ctx) {
  ...
  for (var i = 0; ; i++) {
    ...
    if (layer.type === null || layer.type.equalTo(ultype)) {
      let arity = layer.handle.length;
      if (arity == 1) { // sync
        let o = layer.handle(ctx);
        utils.merge(areq, o);
      } else { // async
...
Server.prototype._respond = function(ctx, notHandled) {
  ...
  var composed = compose(stack, notHandled);
  return composed(ctx);
};
